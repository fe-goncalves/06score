...
type HallView = "titulos" | "premios" | "maiores";

interface TeamHallTabProps {
  profile: TeamProfileData;
}

export function TeamHallTab({ profile }: TeamHallTabProps) {
  ...
  const defaultView: HallView = hasTitulos
    ? "titulos"
    : hasPremios
      ? "premios"
      : "maiores";

  const { tab, setTab } = useClientTab(defaultView, "hallView");
  const activeView: HallView =
  ...
  const resolvedView: HallView = showSwitch ? activeView : "maiores";
  ...
  return (
    <div className="team-hall-tab athlete-info-tab">
      {showSwitch ? (
