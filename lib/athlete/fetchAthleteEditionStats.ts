...
async function fetchAthleteEditionStatsFallback(
  athleteId: string,
  ...
  if (statsError) {
    console.error("[fetchAthleteEditionStats:fallback:stats]", statsError.message);
    return [];
  }
  ...
  if (editionsError) {
    console.error("[fetchAthleteEditionStats:fallback:editions]", editionsError.message);
  }
  if (teamsError) {
    console.error("[fetchAthleteEditionStats:fallback:teams]", teamsError.message);
  }
...
export async function fetchAthleteEditionStats(
  athleteId: string,
): Promise<AthleteEditionStatRow[]> {
  const supabase = getSupabase();
  ...
  if (error) {
    console.error("[fetchAthleteEditionStats:join]", error.message);
    return fetchAthleteEditionStatsFallback(athleteId);
  }
